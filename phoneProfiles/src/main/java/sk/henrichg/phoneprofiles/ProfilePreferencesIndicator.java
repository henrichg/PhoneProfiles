    ...
    public static Bitmap paint(Profile _profile, Context context)
    {
        ...
        if (profile != null)
        {
            if (profile._volumeRingerMode != 0) {
                if (Profile.isProfilePreferenceAllowed(Profile.PREF_PROFILE_VOLUME_RINGER_MODE, null, context).allowed == PreferenceAllowed.PREFERENCE_ALLOWED) {
            ...
            if (profile.getVolumeAlarmChange() ||
                ...
                if ((Profile.isProfilePreferenceAllowed(Profile.PREF_PROFILE_VOLUME_ALARM, null, context).allowed == PreferenceAllowed.PREFERENCE_ALLOWED) ||
                        (Profile.isProfilePreferenceAllowed(Profile.PREF_PROFILE_VOLUME_MEDIA, null, context).allowed == PreferenceAllowed.PREFERENCE_ALLOWED) ||
                        (Profile.isProfilePreferenceAllowed(Profile.PREF_PROFILE_VOLUME_NOTIFICATION, null, context).allowed == PreferenceAllowed.PREFERENCE_ALLOWED) ||
                        (Profile.isProfilePreferenceAllowed(Profile.PREF_PROFILE_VOLUME_RINGTONE, null, context).allowed == PreferenceAllowed.PREFERENCE_ALLOWED) ||
                        (Profile.isProfilePreferenceAllowed(Profile.PREF_PROFILE_VOLUME_SYSTEM, null, context).allowed == PreferenceAllowed.PREFERENCE_ALLOWED) ||
                        (Profile.isProfilePreferenceAllowed(Profile.PREF_PROFILE_VOLUME_VOICE, null, context).allowed == PreferenceAllowed.PREFERENCE_ALLOWED))
                    drawables[countDrawables++] = R.drawable.ic_profile_pref_volume_level;
            }
            // speaker phone
            if (profile._volumeSpeakerPhone != 0) {
                if (Profile.isProfilePreferenceAllowed(Profile.PREF_PROFILE_VOLUME_SPEAKER_PHONE, null, context).allowed == PreferenceAllowed.PREFERENCE_ALLOWED) {
            ...
            if ((profile._soundRingtoneChange == 1) ||
                ...
                if ((Profile.isProfilePreferenceAllowed(Profile.PREF_PROFILE_SOUND_RINGTONE_CHANGE, null, context).allowed == PreferenceAllowed.PREFERENCE_ALLOWED) ||
                        (Profile.isProfilePreferenceAllowed(Profile.PREF_PROFILE_SOUND_NOTIFICATION_CHANGE, null, context).allowed == PreferenceAllowed.PREFERENCE_ALLOWED) ||
                        (Profile.isProfilePreferenceAllowed(Profile.PREF_PROFILE_SOUND_ALARM_CHANGE, null, context).allowed == PreferenceAllowed.PREFERENCE_ALLOWED))
                    drawables[countDrawables++] = R.drawable.ic_profile_pref_sound;
            }
            // sound on touch
            if (profile._soundOnTouch != 0) {
                if (Profile.isProfilePreferenceAllowed(Profile.PREF_PROFILE_SOUND_ON_TOUCH, null, context).allowed == PreferenceAllowed.PREFERENCE_ALLOWED) {
            ...
            if (profile._vibrationOnTouch != 0) {
                if (Profile.isProfilePreferenceAllowed(Profile.PREF_PROFILE_VIBRATION_ON_TOUCH, null, context).allowed == PreferenceAllowed.PREFERENCE_ALLOWED) {
            ...
            if (profile._dtmfToneWhenDialing != 0) {
                if (Profile.isProfilePreferenceAllowed(Profile.PREF_PROFILE_DTMF_TONE_WHEN_DIALING, null, context).allowed == PreferenceAllowed.PREFERENCE_ALLOWED) {
            ...
            if (profile._deviceAutoSync != 0) {
                if (Profile.isProfilePreferenceAllowed(Profile.PREF_PROFILE_DEVICE_AUTOSYNC, null, context).allowed == PreferenceAllowed.PREFERENCE_ALLOWED) {
            ...
            if (profile._deviceWiFiAPPrefs == 1) {
                if (Profile.isProfilePreferenceAllowed(Profile.PREF_PROFILE_DEVICE_WIFI_AP_PREFS, null, context).allowed == PreferenceAllowed.PREFERENCE_ALLOWED)
            ...
            if (profile._deviceLocationServicePrefs == 1) {
                if (Profile.isProfilePreferenceAllowed(Profile.PREF_PROFILE_DEVICE_LOCATION_SERVICE_PREFS, null, context).allowed == PreferenceAllowed.PREFERENCE_ALLOWED)
            ...
            if (profile._deviceScreenTimeout != 0) {
                if (Profile.isProfilePreferenceAllowed(Profile.PREF_PROFILE_DEVICE_SCREEN_TIMEOUT, null, context).allowed == PreferenceAllowed.PREFERENCE_ALLOWED)
                    drawables[countDrawables++] = R.drawable.ic_profile_pref_screen_timeout;
            }
            // lock screen
            if (profile._deviceKeyguard != 0) {
                if (Profile.isProfilePreferenceAllowed(Profile.PREF_PROFILE_DEVICE_KEYGUARD, null, context).allowed == PreferenceAllowed.PREFERENCE_ALLOWED) {
            ...
            if (profile.getDeviceBrightnessChange()) {
                if (Profile.isProfilePreferenceAllowed(Profile.PREF_PROFILE_DEVICE_BRIGHTNESS, null, context).allowed == PreferenceAllowed.PREFERENCE_ALLOWED) {
            ...
            if (profile._deviceAutoRotate != 0) {
                if (Profile.isProfilePreferenceAllowed(Profile.PREF_PROFILE_DEVICE_AUTOROTATE, null, context).allowed == PreferenceAllowed.PREFERENCE_ALLOWED)
                    drawables[countDrawables++] = R.drawable.ic_profile_pref_autorotate;
            }
            // notification led
            if (profile._notificationLed != 0) {
                if (Profile.isProfilePreferenceAllowed(Profile.PREF_PROFILE_NOTIFICATION_LED, null, context).allowed == PreferenceAllowed.PREFERENCE_ALLOWED) {
            ...
            if (profile._headsUpNotifications != 0) {
                if (Profile.isProfilePreferenceAllowed(Profile.PREF_PROFILE_HEADS_UP_NOTIFICATIONS, null, context).allowed == PreferenceAllowed.PREFERENCE_ALLOWED) {
            ...
            */
            ...
            if (profile._devicePowerSaveMode != 0) {
                if (Profile.isProfilePreferenceAllowed(Profile.PREF_PROFILE_DEVICE_POWER_SAVE_MODE, null, context).allowed == PreferenceAllowed.PREFERENCE_ALLOWED) {
            ...
            if (profile._deviceRunApplicationChange == 1) {
                if (Profile.isProfilePreferenceAllowed(Profile.PREF_PROFILE_DEVICE_RUN_APPLICATION_CHANGE, null, context).allowed == PreferenceAllowed.PREFERENCE_ALLOWED)
            ...
            if (profile._deviceCloseAllApplications != 0) {
                if (Profile.isProfilePreferenceAllowed(Profile.PREF_PROFILE_DEVICE_CLOSE_ALL_APPLICATIONS, null, context).allowed == PreferenceAllowed.PREFERENCE_ALLOWED)
                    drawables[countDrawables++] = R.drawable.ic_profile_pref_close_all_applications;
            }
            // force stop application
            if (profile._deviceForceStopApplicationChange == 1) {
                if ((Profile.isProfilePreferenceAllowed(Profile.PREF_PROFILE_DEVICE_FORCE_STOP_APPLICATION_CHANGE, null, context).allowed == PreferenceAllowed.PREFERENCE_ALLOWED) &&
            ...
            if (profile._deviceWallpaperChange == 1) {
                if (Profile.isProfilePreferenceAllowed(Profile.PREF_PROFILE_DEVICE_WALLPAPER_CHANGE, null, context).allowed == PreferenceAllowed.PREFERENCE_ALLOWED)
                    drawables[countDrawables++] = R.drawable.ic_profile_pref_wallpaper;
            }
            // lock device
            if (profile._lockDevice != 0) {
                if (Profile.isProfilePreferenceAllowed(Profile.PREF_PROFILE_LOCK_DEVICE, null, context).allowed == PreferenceAllowed.PREFERENCE_ALLOWED)
